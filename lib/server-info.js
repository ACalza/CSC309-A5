...
function ping_server(ip, port, callback) {
    mc.ping({
        ...
        if (err) {
            ...
            var stat = {
                hostname: (results.description ? (results.description.text ? results.description.text.replace(/\u00A7[0-9a-fk-r]/gi, '').replace(/\s+/mg, ' ') : results.description.replace(/\u00A7[0-9a-fk-r]/gi, '').replace(/\s+/mg, ' ')) : ip),
                from: {
                    address: ip,
                    port: port
                },
